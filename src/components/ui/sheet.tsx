...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 gap-4 bg-[var(--bg-base)] p-6 shadow-xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
        sheetVariants[side],
        className
      )}
      {...props}
    >
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-[var(--text-dim)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-off)] transition-colors focus:outline-none focus:ring-1 focus:ring-[var(--accent-color)] disabled:pointer-events-none">
        <X className="size-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn(
      "text-[18px] font-bold text-[var(--text-primary)]",
      className
    )}
    {...props}
  />
