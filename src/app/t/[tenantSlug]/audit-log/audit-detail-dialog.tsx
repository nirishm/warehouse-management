...
function DiffView({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {/* Old Data */}
      <div>
        <p className="text-xs font-mono uppercase tracking-wider text-[var(--text-dim)] mb-2">
          Previous State
        </p>
        {oldData ? (
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {changedKeys.map((key) => (
              <span
                key={key}
                className="inline-flex items-center rounded-md border border-[var(--accent-color)]/30 bg-[var(--accent-color)]/10 px-2 py-0.5 text-xs font-mono text-[var(--accent-color)]"
              >
                {key}
              </span>
            ))}
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <p className="text-sm font-mono text-[var(--text-dim)] animate-pulse">
                Loading details...
              </p>
            </div>
          ) : (
