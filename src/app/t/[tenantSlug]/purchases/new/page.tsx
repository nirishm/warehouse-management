...
import { usePurchaseForm } from '@/lib/hooks/use-purchase-form';
...
export default function NewPurchasePage() {
  ...
  const mobileSteps: StepConfig[] = [
    {
      label: 'Supplier',
      content: supplierLocationFields,
      validate: form.validateSupplierLocation,
    },
    {
      label: 'Items',
      content: itemsFields,
  ...
  return (
    <>
      ...
      <div className="block md:hidden">
        {form.error && (
          <div className="mx-4 mt-2 rounded-lg border border-[var(--red)]/30 bg-[var(--red-bg)] px-4 py-3 text-sm text-[var(--red)]">
            {form.error}
          </div>
        )}
        <TransactionStepper
          ...
          <form onSubmit={form.handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                variant="orange"
              >
                {form.submitting ? 'Creating...' : 'Create Purchase'}
              </Button>
              <Button
                type="button"
                variant="outline"
                onClick={() => form.router.push(`/t/${tenantSlug}/purchases`)}
